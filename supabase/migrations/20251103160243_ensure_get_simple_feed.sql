-- Ensure get_simple_feed exists (non-destructive)
-- This CREATE OR REPLACE FUNCTION is the same safe simple feed implementation used as fallback.
...
BEGIN
  IF p_feed_type = 'following' THEN
    ...
    OFFSET p_offset;
  ELSE
    -- For You feed: time-bucketed random shuffle (simple & fast)
    RETURN QUERY
    SELECT 
      p.id,
      p.user_id,
      ...
      EXISTS(SELECT 1 FROM saved_posts WHERE post_id = p.id AND user_id = p_user_id) as is_saved
    FROM (
      SELECT *, (EXTRACT(EPOCH FROM created_at)::bigint / 3600 / 6) AS bucket
      FROM posts
      WHERE created_at > NOW() - INTERVAL '30 days'
    ) p
    JOIN profiles prof ON prof.id = p.user_id
    WHERE NOT is_blocked(p_user_id, p.user_id)
    ORDER BY p.bucket, random()
    LIMIT p_limit
    OFFSET p_offset;
  END IF;
END;
